 ...
 * @LastEditors: danae
 * @LastEditTime: 2022-07-26 14:38:38
-->
## 3.1 责任链
- [使用场景]
    - 程序需要使用不同方式处理不同种类请求，且请求类型和顺序未知时
    - 该模式形成一条链，收到请求后，会问询每个处理，都有机会处理请求
...
## 2.7 状态
- [整体说明]
  - 将状态的工作，委派给状态类处理。适合状态多、且复杂的处理，用状态模式替换switch...case的处理方式
- [使用场景]
  - **状态多且逻辑变更频繁** 对象需要根据自身当前状态进行不同行为
  - **大量的条件语句** 将条件分支抽取到相应的状态类的方法中
  - **条件处理有重复代码** 抽象状态基类处理避免重复
- [核心实现]
  - **抽象状态类** 声明状态接口，完全复制上下文类的所有方法
  - **具体状态类** 通过嵌套上下文类，访问其成员变量，并负责更新上下文状态
  - **上下文类** 依赖状态的类，包含状态的成员变量、更新状态方法
- [优缺点]
  - 都是组合机制，将部分工作委派给“帮手”，对象来改变不同情况的行为 
    - **策略** 对象之间相互独立
...
- [使用场景]
  ...
  - **去除条件语句** 委派独立算法对象处理
- [核心实现]
  - **上下文类** 找出修改频率较高的算法，或复杂的条件语句。包含策略对象的成员变量、和相应的设置器
  - **抽象策略类** 声明该算法所有变体的通用策略接口
  - **具体策略类**
  - **客户端** 设计上下文关联的具体策略对象
- [优缺点]
...
## 2.9 访问者
- [整体说明]
  - 避免对已有代码核心逻辑入侵，通过访问者收集各系统的系统，用于生成运营报表、日志追踪等
- [使用场景]
  - **对多个目标类执行系统操作**
  - **清理辅助业务逻辑，更专注主要工作**
  - **某个行为仅在类层次结构中有意义**
- [核心实现]
    - **抽象访问者** 声明一组访问方法，对应要访问的具体元素类
    - **具体元素类** 实现接收方法，实现将自身作为参数调用访问者，以便访问者获取元素对象数据，进行相应处理
